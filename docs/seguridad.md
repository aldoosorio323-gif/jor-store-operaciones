...
Las Etapas 1, 2 y 3 están conectadas y validadas con Supabase real; las migraciones 001–004 están aplicadas local y remotamente. La Etapa 4 está implementada en código y la migración 005 está pendiente de revisión y aplicación remota. Los datos de clientes permanecen detrás de autenticación y RLS; no se incluyen en auditoría ni logs. Administrador y operador activos gestionan clientes, pedidos y pagos, pero activación de clientes, reembolsos y conciliación financiera son exclusivos del administrador. No hay políticas DELETE ni CRUD normal con `service_role`. Las pruebas SQL y de concurrencia real continúan pendientes. La Etapa 5 no ha sido iniciada.

...
Las Etapas 1–4 están conectadas y validadas con Supabase real; las migraciones 001–005 están aplicadas local y remotamente. La Etapa 5A está implementada en código y la migración 006 está pendiente de revisión y aplicación remota. Destinatarios, contactos de transportistas y direcciones permanecen detrás de autenticación y RLS y no se copian a auditoría. Administrador y operador activos gestionan envíos; solo el administrador mantiene transportistas, cancela envíos sensibles y ejecuta conciliación. No hay políticas DELETE ni CRUD normal con `service_role`. Las pruebas SQL y de concurrencia real de logística continúan pendientes.

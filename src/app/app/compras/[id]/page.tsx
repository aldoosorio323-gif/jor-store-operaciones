...
export default async function PurchaseDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <section><Link href="/app/compras" className="text-sm font-semibold text-emerald-800">← Volver a compras</Link>
    ...
    <div className="mt-8 flex items-center justify-between"><h2 className="text-xl font-semibold">Líneas</h2><div className="text-right text-sm"><p>Subtotal {money.format(purchase.subtotal)}</p><p>Impuesto {money.format(purchase.taxAmount)}</p><p className="font-semibold">Total {money.format(purchase.totalAmount)}</p></div></div>
    ...
    {editable ? <div className="mt-6 rounded-2xl border border-dashed border-emerald-300 bg-emerald-50/40 p-5"><h2 className="font-semibold">Añadir línea</h2><div className="mt-4"><PurchaseItemForm purchaseId={purchase.id} variants={variants} nextLine={nextLine} /></div></div> : null}
  </section>;
}

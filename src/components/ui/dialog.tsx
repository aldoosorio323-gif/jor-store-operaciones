...
export function Modal({ open, onClose, title, description, children, footer, danger = false }: { open: boolean; onClose: () => void; title: string; description?: string; children?: ReactNode; footer?: ReactNode; danger?: boolean }) {
  ...
  return createPortal(<div className="fixed inset-0 z-[100] flex items-end justify-center bg-[var(--app-overlay)] p-0 backdrop-blur-[1px] sm:items-center sm:p-4" onMouseDown={(event)=>{if(event.target===event.currentTarget)onClose();}}><div ref={panelRef} role="dialog" aria-modal="true" aria-labelledby={titleId} aria-describedby={description ? descriptionId : undefined} className="app-surface max-h-[92vh] w-full overflow-y-auto rounded-t-2xl border shadow-2xl sm:max-w-lg sm:rounded-2xl"><div className="app-border flex items-start justify-between gap-4 border-b px-5 py-4"><div><h2 id={titleId} className="app-text text-lg font-semibold">{title}</h2>{description ? <p id={descriptionId} className="app-muted mt-1 text-sm leading-5">{description}</p> : null}</div><button type="button" onClick={onClose} className="app-muted -mr-2 rounded-lg p-2 hover:bg-[var(--app-surface-soft)] hover:text-[var(--app-text)]" aria-label="Cerrar diálogo"><Icon name="close" className="size-5"/></button></div>{children ? <div className="px-5 py-5">{children}</div> : null}{footer ? <div className={`app-border app-soft flex flex-col-reverse gap-2 border-t px-5 py-4 sm:flex-row sm:justify-end ${danger ? "border-t-red-300" : ""}`}>{footer}</div> : null}</div></div>, document.body);
}

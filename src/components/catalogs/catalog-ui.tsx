...
export function CatalogFiltersForm({ basePath, filters, isAdministrator, placeholder }: { basePath: string; filters: CatalogFilters; isAdministrator: boolean; placeholder: string }) {
  return <form action={basePath} className="mt-5"><FilterBar className="grid gap-3 sm:grid-cols-[minmax(16rem,1fr)_12rem_auto] sm:items-end"><label className="app-text text-sm font-medium">Buscar<input name="query" defaultValue={filters.query} placeholder={placeholder} className={fieldClass}/></label>{isAdministrator ? <label className="app-text text-sm font-medium">Estado<select name="status" defaultValue={filters.status} className={fieldClass}><option value="active">Activos</option><option value="inactive">Inactivos</option><option value="all">Todos</option></select></label> : null}<button className={buttonStyles.primary}>Aplicar filtros</button></FilterBar></form>;
}

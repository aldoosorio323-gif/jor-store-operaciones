...
export type OrderListItem = { id:string; orderNumber:string; customerId:string; customerName:string; status:OrderStatus; paymentStatus:OrderPaymentStatus; orderedAt:string; totalAmount:number; paidAmount:number };
export type OrderItem = { id:string; orderId:string; lineNumber:number; variantId:string; variantLabel:string; warehouseId:string; locationId:string; locationLabel:string; quantity:number; reservedQuantity:number; dispatchedQuantity:number; returnedQuantity:number; unitPrice:number; discountAmount:number; taxAmount:number; lineTotal:number };
export type OrderDetail = OrderListItem & { notes:string|null; items:OrderItem[] };
export type PaymentListItem = { id:string; paymentNumber:string; orderId:string; orderNumber:string; amount:number; method:PaymentMethod; status:PaymentStatus; paidAt:string|null; reference:string|null; notes:string|null };
